...
module.exports = class Log {
  ...
  static async insert(log) {
    const {
      ...
      "INSERT into recipes (date_of_event, notes, rating) VALUES ($1, $2, $3) RETURNING *",
  ...
  static async findById(id) {
    ...
    if (!rows[0])
      throw new Error(
        JSON.stringify({
          status: 404,
          message: `No log with id ${id} found.`,
        })
      );
  ...
  static async delete(id) {
    const {
      rows,
    } = await pool.query("DELETE FROM logs WHERE id=$1 RETURNING *", [id]);
